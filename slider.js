/*
 * js touch slider plugin v.1.0
 * @author anwentian, github:https://github.com/anwentian
*/
function Slider(options){
    this.default ={banner: "banner", width: 320, height:180,autoSlide:true};
...
Slider.prototype.initial = function(){
    this.banner.style.width = this._options.width+"px";
    this.banner.style.height =  this._options.height+"px";
    this.carosel.style.width = this.itemsCount * this._options.width+"px";
    for(let i =0;i<this.itemsCount;i++){
        this.carouselItem[i].style.width = this._options.width+"px";
        this.carouselItem[i].style.height = this._options.height+"px";
...
Slider.prototype.autoPlay = function(){
    if(this._options.autoSlide == false) return;
    clearTimeout(this.timer);
    let transform = "translate3d("+-this._options.width*this.target+"px, 0px,0px)";
...
Slider.prototype.touchEvent = function(){
    ...
    this.banner.addEventListener("touchmove",function(ev){ 
        ...
        if( 0 < _this.currentX < 320){
            _this.carosel.style['-webkit-transform']="translate3d("+(-_this._options.width*_this.current+_this.dis)+"px, 0px,0px)";
            _this.carosel.style.transform = "translate3d("+(-_this._options.width*_this.current+_this.dis)+"px, 0px,0px)";
    ...
    this.banner.addEventListener("touchend",function(ev){
       clearTimeout(_this.timer1);
        ...
        if(_this.dis > _this.minDis){
            _this.target = _this.current-1;
            _this.current==0?_this.target=0:_this.target;
        } else if (_this.dis <  -_this.minDis) {
            _this.target = _this.current+1;
            _this.current==2?_this.target=2:_this.target;
        }

        _this.carosel.style['-webkit-transform']="-translate3d("+-(_this._options.width*_this.target)+"px, 0px,0px)";
        _this.carosel.style.transform = "translate3d("+-(_this._options.width*_this.target)+"px, 0px,0px)";
...
Slider.prototype.indicatorHandle = function(){
    ...
    for (let i = 0;i<this.itemsCount;i++) {
        ...
        this.indicators[i].addEventListener("click",function(){          
            _this.current=i;
            let transform = "translate3d("+-_this._options.width*_this.current+"px, 0px,0px)";           
            let duration = "300ms";
